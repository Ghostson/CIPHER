...
{B}[{P}>{B}]{P} {K}AU {P}: {K}G H O S T S O N- {P}D A R L I N G T O N. {H}GHOST👻 {M}SOLUTION 🔥
{B}[{P}>{B}]{P} {B}FB {P}: {B}G H O S T S O N.
{B}[{P}>{B}]{P} {U}IG {P}: {U}@ANONYMOUS
{B}[{P}>{B}]{P} {H}WA {P}: {H}+2349138017570
{B}[{P}\x1b[1;92mâ ââ{B}]{M}\x1b[1;93mââââââââââââââââââââââââââââââââââââââ{P}
""")
# Login Cookie
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        print(f"{B}[{P}!{B}]{P} {H}Wait for a minute...{P}");sleep(3);os.system('xdg-open https://wa.me/+2349138017570?text=Hello');exit()
    elif ___cookie in ['', ' ']:
